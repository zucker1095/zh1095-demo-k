...
class CCount {
   ...
   *
   * <p>dp[i][j] 表示从 0 出发走 j 步到达 i 点的方案，即排列数
   *
...
class SSub {
   ...
   * <p>[n-1,0] & [m-1,0]
   *
   * <p>由于是连续，因此递推关系只依赖前一个变量，所以可以状态压缩为一维
   *
  ...
  public int longestConsecutive(int[] nums) {
    ...
    for (int num : nums) {
      ...
      int left = map.getOrDefault(num - 1, 0), right = map.getOrDefault(num + 1, 0);
  ...
  public int minDistance(String word1, String word2) {
    return deleteOperationForTwoStrings(word1, word2);
  }
      ...
      for (int j = 1; j <= n2; j++) {
        dp[i][j] =
            (s1.charAt(i - 1) == s2.charAt(j - 1))
                ? Math.min(Math.min(dp[i - 1][j], dp[i][j - 1]) + 1, dp[i - 1][j - 1])
